...
fn swapfs_format_writes_superblock_and_zero_metadata_blocks() {
    ...
    assert_eq!(sb.meta_start_block, 2);
    assert_eq!(sb.meta_block_count, 2);
    assert_eq!(sb.data_start_block, 4);
...
fn swapfs_mount_loads_existing_superblock_and_metadata() {
    ...
    write_first_meta(&disk, SwapFsMetaDisk::new_used("keep", 3, 1, 4).unwrap());
...
fn swapfs_mount_or_format_preserves_existing_metadata() {
    ...
    let mut block = [0u8; SWAPFS_BLOCK_SIZE];
    assert_eq!(disk.read_block(2, &mut block), Ok(()));
...
fn swapfs_read_write_meta_uses_metadata_index_location() {
    ...
    disk.read_block(2, &mut block_one).unwrap();
    disk.read_block(3, &mut block_two).unwrap();
...
#[test]
fn swapfs_bitmap_allocation_survives_remount() {
    let disk = Arc::new(Disk::new("swap0", 32, SWAPFS_BLOCK_SIZE));
    let fs = SwapFs::format(disk.clone(), 32, 4).unwrap();

    assert_eq!(fs.alloc_blocks(5), Ok(3));

    let remounted = SwapFs::mount(disk).unwrap();
    assert_eq!(remounted.alloc_blocks(1), Ok(8));
}

fn write_first_meta(disk: &Disk, meta: SwapFsMetaDisk) {
    let mut super_block = [0u8; SWAPFS_BLOCK_SIZE];
    disk.read_block(0, &mut super_block).unwrap();
    let sb = SwapFsSuperBlockDisk::decode_from(&super_block).unwrap();

    let mut block = [0u8; SWAPFS_BLOCK_SIZE];
    disk.read_block(sb.meta_start_block, &mut block).unwrap();
    meta.encode_into(&mut block[0..SWAPFS_META_DISK_SIZE])
        .unwrap();
    disk.write_block(sb.meta_start_block, &block).unwrap();
}
